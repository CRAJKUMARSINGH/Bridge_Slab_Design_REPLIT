...
// Function to perform comprehensive analysis of all sheets in the template
async function comprehensiveTemplateAnalysis() {
  try {
    ...
    const template = XLSX.readFile(templatePath, { 
      ...
      cellHTML: false, // HTML rendering
      bookVBA: true, // VBA macros if any
      bookFiles: true, // Internal files
      bookProps: true, // Properties
      bookSheets: true // Sheet names
    });
    ...
    for (let i = 0; i < template.SheetNames.length; i++) {
      ...
      // Skip problematic sheets that might cause errors
      if (!sheet) {
      ...
      if (sheet['!merges'] && sheet['!merges'].length > 0) {
        ...
        // Show first 3 merged ranges
        sheet['!merges'].slice(0, 3).forEach((range, index) => {
          console.log(`    ${index + 1}. ${XLSX.utils.encode_range(range)}`);
        });
        
        if (sheet['!merges'].length > 3) {
          console.log(`    ... and ${sheet['!merges'].length - 3} more`);
        }
      }
...
<body>
    <div class="container">
        ...
        <h2>📋 DETAILED SHEET ANALYSIS</h2>
        ${Object.keys(analysisReport.sheets).map(sheetName => {
          ...
          <div class="sheet-section">
              ...
              <div class="sheet-content">
                  ...
                  <table>
                      ...
                      ${sheet.sampleHeaders.map(header => `
                      <tr>
                  ...
                  <table>
                      ...
                      ${sheet.sampleFormulas.map(formula => `
                      <tr>
        ...
        <h2>🔑 KEY SHEETS ANALYSIS</h2>
        ${['INSERT- HYDRAULICS', 'INSERT C1-ABUT', 'INSERT ESTIMATE', 'STABILITY CHECK FOR PIER'].map(sheetName => {
          if (template.Sheets[sheetName]) {
            const sheet = template.Sheets[sheetName];
            return `
            <div class="sheet-section">
                <div class="sheet-header">${sheetName}</div>
                <div class="sheet-content">
                    <p><strong>Range:</strong> ${sheet['!ref'] || 'undefined'}</p>
                    <p><strong>Merged Cells:</strong> ${sheet['!merges'] ? sheet['!merges'].length : 0} ranges</p>
                    ${sheet['!merges'] && sheet['!merges'].length > 0 ? `
                    <p><strong>First 5 Merged Ranges:</strong></p>
                    <ul>
                        ${sheet['!merges'].slice(0, 5).map(range => `<li>${XLSX.utils.encode_range(range)}</li>`).join('')}
                    </ul>
                    ` : ''}
                </div>
            </div>
  ...
  const outputPath = path.join(__dirname, 'OUTPUT', 'comprehensive_template_analysis.html');
...
function verifyTemplatePreservation(template) {
  ...
  try {
    ...
    const verificationPath = path.join(__dirname, 'OUTPUT', 'template_preservation_verification.xlsx');
